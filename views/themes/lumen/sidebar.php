<div class="col-lg-3 col-md-3 col-sm-12 sidebar">
	<h5 class="page-title mt-3 mb-3">Youtube Sekolah</h5>
	...
	<!-- <?php $links = get_links();
		...
		<div class="list-group">
			<?php foreach ($links->result() as $row) { ?>
				<a href="<?= $row->link_url ?>" class="list-group-item list-group-item-action " target="<?= $row->link_target ?>"><?= $row->link_title ?></a>
			<?php } ?>
		</div>
	<?php } ?> -->

	<?php $query = get_archives(date('Y'));
	if ($query->num_rows() > 0) { ?>
		<h5 class="page-title mt-3 mb-3">Arsip <?= date('Y') ?></h5>
		<?php foreach ($query->result() as $row) { ?>
			<a href="<?= site_url('arsip/' . date('Y') . '/' . $row->code) ?>" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center  mb-1">
				<?= bulan($row->code) ?>
	...
	<!-- Paling Dikomentari -->
	<?php $query = get_most_commented(5);
	if ($query->num_rows() > 0) { ?>
		<h5 class="page-title mt-3 mb-3">Paling Dikomentari</h5>
		<div class="list-group mt-3 mb-3">
			<?php foreach ($query->result() as $row) { ?>
				<a href="<?= site_url('read/' . $row->id . '/' . $row->post_slug) ?>" class="list-group-item list-group-item-action ">
	...
	if ($query) { ?>
		...
		<div class="card  border border-secondary mb-3">
			...
			<div class="card-footer">
				<div class="btn-group">
					<button type="button" name="button" onclick="vote(); return false;" class="btn action-button "><i class="fa fa-send"></i> Submit</button>
					<a href="<?= site_url('hasil-jajak-pendapat') ?>" class="btn action-button "><i class="fa fa-bar-chart"></i> Hasil</a>
				</div>
			</div>
		</div>
	<?php } ?>
	...
	<form class="card p-1 border border-secondary mt-2 mb-2 ">
		<div class="input-group">
			<input type="text" id="subscriber" onkeydown="if (event.keyCode == 13) { subscribe(); return false; }" class="form-control  border border-secondary" placeholder="Email Address...">
			<div class="input-group-append">
				<button type="button" onclick="if (event.keyCode == 13) { subscribe(); return false; }" class="btn action-button "><i class="fa fa-envelope"></i></button>
			</div>
		</div>
	</form>
