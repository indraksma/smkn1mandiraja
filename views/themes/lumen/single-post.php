...
<div class="col-lg-9 col-md-9 col-sm-12 ">
	...
	<!-- Get Anther Posts -->
	<?php $query = $post_type == 'post' ? get_related_posts($query->post_categories, _toInteger($this->uri->segment(2))) : get_another_pages(_toInteger($this->uri->segment(2)), 5);
	if ($query->num_rows() > 0) { ?>
			...
			<div class="card rounded-0 border border-secondary mb-3">
				<div class="card-body p-3">
					...
					<div class="d-flex justify-content-between align-items-center mt-1">
						...
						<a href="<?= site_url('read/' . $row->id . '/' . $row->post_slug) ?>" class="btn btn-sm action-button"><i class="fa fa-search"></i></a>
					</div>
				</div>
			</div>
		<?php } ?>
...
<?php $this->load->view('themes/lumen/sidebar') ?>